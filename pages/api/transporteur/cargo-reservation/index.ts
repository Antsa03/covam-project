...
// GET /api/transporteur/cargo-reservation?page=1&limit=20&status=...
export default async function handler(
  ...
  try {
    ...
    const pbIds = transporteur.transports.flatMap((t) =>
      t.pb_transports.map((pb) => pb.id_pb_transport),
    );

    const where: Record<string, unknown> = {
      reservation: { pb_transport_id: { in: pbIds } },
    };
    ...
    console.error("[GET /api/transporteur/cargo-reservation]", error);
